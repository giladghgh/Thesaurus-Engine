...
TheCracks = ["have", "has", "had", "be", "is", "was", "were", "are", "been", "do", "does", "did"]
CracksDict = {'have': 'have', 'has': 'have', 'had': 'have', 'be': 'be', 'is': 'be', 'was': 'be', 'were': 'be', 'are': 'be', 'been': 'be', 'do': 'do', 'does': 'do', 'did': 'do'}
...
def lemmatise(wordtags):
    ...
    for word,tag in wordtags:
        ...
        # the following do not get replaced as WordNet can't handle these properly:
...
def inflect(wordtags):
    """ Inflect/conjugate list of (word,tag) tuples correctly. """
    ...
    for i,(word,tag) in enumerate(wordtaglist):
        if word in ["a","an"]:
            ...
            yield inflected,tag
            continue
        elif (tag in ["NNP"]) or (word in TheCracks) or (len(word) < 3):
            yield word,tag
            continue
        ...
        if " " not in word:
            ...
            # e.g. "run across"  ->  "ran across"  RATHER THAN  "run acrossed"
...
def assemble(wordtags):
    """ Re-assemble the text, compliant to certain heuristics and basic English grammar """
    first = True
    ...
    for word,tag in wordtags:
        ...
        isContraction = hasPunct and not bool( (wordset - {"'"}) & PunctSet ) and ' ' not in word
...
if __name__ == '__main__':
    prompt = "Enter text below so I can synonymise it, or enter \"file\" (no quotes) to read from 'input.txt':\n"
    which = input(prompt)
    if which.lower() == "file":
